...
export class ConfigListComponent {
    ...
    combineLatest([toObservable(this.searchTerm), toObservable(this.refreshNonce)]).pipe(
      switchMap(([q]) => {
        ...
        const request$ = trimmed
          ? this.configService.search({ search: trimmed, page: 0, limit: 50 })
          : this.configService.list();
        return timer(trimmed ? 300 : 0).pipe(
          switchMap(() =>
            request$.pipe(
              catchError(() => {
                this.error.set('Could not load configurations.');
  ...
  deleteConfig(item: SystemConfig): void {
    this.deleteError.set(null);
    const label = item.cfg_key?.trim() || `#${item.id}`;
    if (!window.confirm(`Delete configuration "${label}"?`)) {
      return;
    }
    ...
    this.configService
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          this.deleteError.set(formatHttpError(err, 'Could not delete configuration.'));
        },
      });
  }
}
