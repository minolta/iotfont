...
call "%~dp0remote-auth.bat" "%SSH_TARGET%" "cd %IOT_REMOTE_DIR% && docker load -i font.tar && docker rm -f iot-font 2>/dev/null || true && docker network inspect iot-net >/dev/null 2>&1 || docker network create iot-net && docker run -d --name iot-font --network iot-net --restart unless-stopped -p 8080:80 iot-font:latest"
if errorlevel 1 exit /b 1
