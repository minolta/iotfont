  ...
  template: `
    <div class="chart-card" aria-labelledby="chart-heading">
      <div class="chart-header">
        <h2 id="chart-heading">Humidity &amp; temperature</h2>
        @if (loading()) {
      ...
      @if (hasData()) {
        <div class="chart-wrap">
          <canvas #canvas aria-label="Humidity and temperature line chart"></canvas>
        </div>
...
export class HumidityChartComponent implements AfterViewInit, OnDestroy {
  readonly readings = input.required<Humidity[]>();
  ...
  private buildPoints(readings: Humidity[]): ChartPoint[] {
    return [...readings]
      ...
      .map((reading) => ({
        ...
        humidity: this.toNumber(reading.h),
        temperature: this.toNumber(reading.t),
      }));
  }
